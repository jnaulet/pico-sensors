...
static int ht16k33_write_wait(struct ht16k33 *ctx, int cc, /*@null@*/ const void *buf, size_t n)
{
...
static void display_main(void *priv)
{
    ...
    for (;;) {
        ...
        static uint16_t fb[] = { 0, 0,  0,     0,
                                 ...
                                 0, 0,  0,     0 };
#if 1
        (void)ht16k33_write_wait(&ht16k33, HT16K33_CC_DISPLAY_DATA, &fb[i], (size_t)8);
        if (++i == 12) i = 0; /* reset scroll */
#else
        uint16_t data = (uint16_t)(1 << i);
        (void)ht16k33_write_wait(&ht16k33, HT16K33_CC_DISPLAY_DATA, &data, sizeof(data));
